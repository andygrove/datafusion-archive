    ...
    fn execute(&mut self, args: &[Value]) -> Result<()> {
        ...
        match args[0] {
            Value::Column(ref array) => {
                ...
                match array.data() {
                    ArrayData::Boolean(ref buf) => max_in_column!(self, buf, bitmap, Boolean),
                    ArrayData::UInt8(ref buf) => max_in_column!(self, buf, bitmap, UInt8),
                    ArrayData::UInt16(ref buf) => max_in_column!(self, buf, bitmap, UInt16),
                    ArrayData::UInt32(ref buf) => max_in_column!(self, buf, bitmap, UInt32),
                    ArrayData::UInt64(ref buf) => max_in_column!(self, buf, bitmap, UInt64),
                    ArrayData::Int8(ref buf) => max_in_column!(self, buf, bitmap, Int8),
                    ArrayData::Int16(ref buf) => max_in_column!(self, buf, bitmap, Int16),
                    ArrayData::Int32(ref buf) => max_in_column!(self, buf, bitmap, Int32),
                    ArrayData::Int64(ref buf) => max_in_column!(self, buf, bitmap, Int64),
                    ArrayData::Float32(ref buf) => max_in_column!(self, buf, bitmap, Float32),
                    ArrayData::Float64(ref buf) => max_in_column!(self, buf, bitmap, Float64),
...
mod tests {
    ...
    fn test_max_with_nulls() {
        let mut max = MaxFunction::new(&DataType::Int8);
        assert_eq!(DataType::Int8, max.return_type());
        ...
        max.execute(&vec![Value::Column(Rc::new(arr))])
            .unwrap();
        let result = max.finish().unwrap();
